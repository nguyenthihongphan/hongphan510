 ...
 <div id="admin-heading-panel">
  ...
  <table width="600" border="1" align="center">
    ...
    <tr>
      ...
      <input name="txtten" type="text" id="txtten" value="<?php echo $p->chonlay("select ten from taikhoan where id='$layid' limit 1")?>" />
    ...
    <tr>
      ...
      <input name="txttk" type="text" id="txttk" value="<?php echo $p->chonlay("select username from taikhoan where id='$layid' limit 1")?>" /></td>
    </tr>
    <tr>
      <td>Mật khẩu</td>
      <td align="left"><label for="txtpass"></label>
      <input name="txtpass" type="text" id="txtpass" value="<?php echo $p->chonlay("select password from taikhoan where id='$layid' limit 1")?>" /></td>
    </tr>
    <tr>
      <td>Phân quyền</td>
      <td align="left"><label for="txtphanquyen"></label>
      <input name="txtphanquyen" type="text" id="txtphanquyen" value="<?php echo $p->chonlay("select phanquyen from taikhoan where id='$layid' limit 1")?>" /></td>
    </tr>
    <tr>
...
switch($_POST['nut'])
{
	...
	{
		$ten=$_REQUEST['txtten'];
		$tentaikhoan=$_REQUEST['txttk'];
		$pass=$_REQUEST['txtpass'];
		$phanquyen=$_REQUEST['txtphanquyen'];
		if($tentaikhoan>0)
		{
				if($p->themxoasua("INSERT INTO taikhoan(ten,username,password,phanquyen)VALUES( '$ten', '$tentaikhoan', '$pass','$phanquyen')")==1)
				{
	...
	case'Xóa tài khoản';
	{
		...
		if($idxoa>0)
		{
			if($p->themxoasua("delete from taikhoan where id='$idxoa' limit 1")==1)
			{
	...
	case'Sửa tài khoản';
	{
		...
		if($idsua>0)
		{
			if($p->themxoasua("UPDATE taikhoan SET ten='$ten', username= '$tentaikhoan', password='$pass',phanquyen='$phanquyen' WHERE id='$idsua' LIMIT 1")==1)
				{
...
<hr/>
<?php
$p->load_ds_taikhoan("select * from taikhoan order by id desc")
?>
